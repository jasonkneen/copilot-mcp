...
export function registerToolUserChatParticipant(context: vscode.ExtensionContext) {
    const handler: vscode.ChatRequestHandler = async (request: vscode.ChatRequest, chatContext: vscode.ChatContext, stream: vscode.ChatResponseStream, token: vscode.CancellationToken) => {
        ...
        const result = await renderPrompt(
            ToolUserPrompt as unknown as PromptElementCtor<ToolUserProps, any>,
            {
                context: chatContext,
                request,
                toolCallRounds: [],
    ...
    const toolUser = vscode.chat.createChatParticipant('copilot-mcp.tools', handler);
    toolUser.iconPath = { id: 'tools', color: new vscode.ThemeColor('chat') };
